...
layui.define(['form', 'jquery'], function (exports) { //提示：模組也可以依賴其他模組，如：layui.define('layer', callback);
    ...
    view = {
      ...
      makeNodeLineClass: function (setting, node) {
        ...
        if (setting.view.showLine) {
          if (node.level == 0 && node.isFirstNode && node.isLastNode && !node.isParent) {//由於單個父節點不顯示三角圖示，所以添加了 && !node.isParent 條件，如果在其他的方有影響修改是請考慮這個問題。
...
(function ($) {
  ...
  TreeSelect.prototype.render = function (options) {
    var elem = options.elem,
        ...
        // 占位符（提示資訊）
        placeholder = options.placeholder === undefined ? '請選擇' : options.placeholder,
        // 是否開啟搜索
  ...
  /**
   * 選中節點，因為tree是非同步加載，所以必須在success回調中調用checkNode函數，否則無法獲取生成的DOM元素
