function Tag(inputId){
	...
	obj.initView=function(){
		...
		appendStr+='<div class="tagList"></div><input type="text" class="tagInput" style="display:none" />';
		appendStr+='</div>';
...
var tagTake ={
	...
	"getTagItemModel":function(valueStr){
		var text = valueStr.split('_')[0];
		var val = valueStr.split('_').length>1?valueStr.split('_')[1]:valueStr.split('_')[0];
		var v = '<div class="tagItem">'
				+'<span>'+text+'</span>'
