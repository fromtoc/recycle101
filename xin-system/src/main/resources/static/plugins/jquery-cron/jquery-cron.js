...
(function($) {
    ...
    var combinations = {
        ...
        "分钟" : /^0\s(0\/1)\s(\*\s){3}\?$/,             // "0 0/1 * * * ?"
        "小时"   : /^0\s\d{1,2}\s(0\/1)\s(\*\s){2}\?$/,    // "0 - 0/1 * * ?"
        "天"    : /^0\s(\d{1,2}\s){2}(\*\s){2}\?$/,       // "0 - - * * ?"
        "周"   : /^0\s(\d{1,2}\s){2}\?\s(\*\s)\d{1,2}$/, // "0 - - ? * -"
        "月"  : /^0\s(\d{1,2}\s){3}\*\s\?$/,            // "0 - - - * ?"
        "年"   : /^0\s(\d{1,2}\s){4}\?\s\*$/             // "0 - - - - ? *"
    };
    ...
    var methods = {
        init : function(opts) {
            ...
            block["month"] = $("<span class='cron-block cron-block-month'>"
                    + " <select name='cron-month'>" + str_opt_month
                    + "</select> </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["mins"] = $("<span class='cron-block cron-block-mins'>"
                    + " <select name='cron-mins'>" + str_opt_mih
                    + "</select> 分 </span>")
                .appendTo(this)
                .data("root", this);
