/*
* @Author: layui-2
* @Date:   2018-08-31 11:40:42
* @Last Modified by:   layui-2
* @Last Modified time: 2018-09-04 14:44:38
*/
layui.define(['jquery','layer'],function(exports){
  "use strict";
  var $ = layui.jquery,layer = layui.layer
  

  //外部介面
  ,inputTags = {
    config: {}

    //設置全局項
    ,set: function(options){
      var that = this;
      that.config = $.extend({}, that.config, options);
      return that;
    }

    // 事件監聽
    ,on: function(events, callback){
      return layui.onevent.call(this, MOD_NAME, events, callback)
    },
  }

   //操作當前實例
  ,thisinputTags = function(){
    var that = this
    ,options = that.config;

    return {
      config: options
    }
  }

  //字元常量
  ,MOD_NAME = 'inputTags'


  // 構造器
  ,Class = function(options){
    var that = this;
    that.config = $.extend({}, that.config, inputTags.config, options);
    that.render();
  };

   //默認配置
  Class.prototype.config = {
    close: false  //默認:不開啟關閉按鈕
    ,theme: ''   //背景:顏色
    ,content: [] //默認標籤
    ,aldaBtn: false //默認配置
	
  };

  // 初始化
  Class.prototype.init = function(){
    var that = this
    ,spans = ''
    ,options = that.config
    ,span = document.createElement("span"),
    spantext = $(span).text("獲取全部數據").addClass('albtn');
    if(options.aldaBtn){
      $('body').append(spantext)
    }
    
    $.each(options.content,function(index,item){
      var id = item.indexOf('@')>-1?item.split('@')[0]:item;
      var name = item.indexOf('@')>-1?item.split('@')[1]:item;
      var span = document.querySelector('span[data-id="'+id+'"]');
      if (span==null) {
        if (options.close) {
          spans +='<span data-id="'+id+'"><em>'+name+'</em><button type="button" class="close">×</button></span>';
        } else {
          spans +='<span data-id="'+id+'"><em>'+name+'</em></span>';
        }
      }
    })
    options.elem.before(spans)
    that.events()
  }

  Class.prototype.render = function(){
    var that = this
    ,options = that.config
    options.elem = $(options.elem);
    that.enter()
  };

	// 用戶自定義
  Class.prototype.del = function(id){
    var that = this
        ,spans = ''
        ,options = that.config;
    var Thisremov = $(id).remove(),
        ThisText = $(Thisremov).find('em').text();
    options.content.splice($.inArray(ThisText,options.content),1)


  };


  // 回車生成標籤
  Class.prototype.enter = function(){
    var that = this
    ,spans = ''
    ,options = that.config;
    options.elem.focus();
    options.elem.keypress(function(event){  
      var keynum = (event.keyCode ? event.keyCode : event.which);  
      if(keynum == '13'){  
        var $val = options.elem.val().trim();
        if(!$val) return false;
        if(options.content.indexOf($val) == -1){
          options.content.push($val)
          that.render()
          spans ='<span><em>'+$val+'</em><button type="button" class="close">×</button></span>';
          options.elem.before(spans)
        }
        options.done && typeof options.done === 'function' && options.done($val);
        options.elem.val('');
      }   
    })
  };
  
  //事件處理
  Class.prototype.events = function(){
     var that = this
    ,options = that.config;
    $('.albtn').on('click',function(){
      console.log(options.content)
    })
    $('#tags').on('click','.close',function(){
      var Thisremov = $(this).parent('span').remove(),
      ThisText = $(Thisremov).find('em').text();
      options.content.splice($.inArray(ThisText,options.content),1)
    })
  };

  //核心入口
  inputTags.render = function(options){
    var inst = new Class(options);
    inst.init();
    return thisinputTags.call(inst);
  };

  inputTags.del = function(id){
    document.querySelector('span[data-id="'+id+'"]').remove()
  };

  exports('inputTags',inputTags);
}).link(basePath + '/plugins/layui/2.4.3/css/inputTags.css')