    ...
    view = {
      ...
      makeNodeLineClass: function (setting, node) {
        ...
        if (setting.view.showLine) {
          if (node.level == 0 && node.isFirstNode && node.isLastNode && !node.isParent) {//由于单个父节点不显示三角图标，所以添加了 && !node.isParent 条件，如果在其他的方有影响修改是请考虑这个问题。
...
(function ($) {
  ...
  var obj = {
    filter: function(filter){
      if (!filter) {
        layui.hint().error('filter can not be empty!');
      }
