...
(function(window, document, $) {
    window.utils = {
            ...
            /**
             * 通过key 获取参数中的值
             * @param String name like this:  name=123&pwd=456
             */
            ...
            toTreeData:function (data, attributes) {
            	  ...
            	  for (let i = 0; i < resData.length; i++) {
            	    if (resData[i].parentId === attributes.rootId) {
            	      let obj = {
            	        ...
            	        level:resData[i][attributes.level],
            	        ...
            	        for (let j = 0; j < resData.length; j++) {
            	          if (chiArr[i].id === resData[j][attributes.parentId]) {
            	            let obj = {
            	              ...
            	              level:chiArr[i].level+1,
            	              parentId:resData[j][attributes.parentId],
            	              //children: []
            	            };
            	            if (chiArr[i].children==null || chiArr[i].children==undefined){
            	...
            	toTreeDataNew:function (data, attributes) {
              	  let resData = data;
              	  let tree = [];
              	  if (attributes==null){
              		  attributes = {
              	  ...
              	  function run(chiArr) {
              	      for (let i = 0; i < chiArr.length; i++) {
              	        ...
              	        if(chiArr[i].children==null || chiArr[i].children==undefined){
              	        	chiArr.splice(i, 1);
              	        	i--;
              	        }else if(chiArr[i].children.length > 0 && chiArr[i].children !== null){
              	        	run(chiArr[i].children);
              	        }
              	      }
              	  }
